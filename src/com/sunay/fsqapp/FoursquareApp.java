...
import com.sunay.fsqapp.FoursquareDialog.FsqDialogListener;
...
public class FoursquareApp {
     ...
     */
    public static final String CALLBACK_URL = "https://sunayapp.com/connect";
	...
	mTokenUrl = TOKEN_URL + "&client_id=" + clientId + "&client_secret=" + clientSecret + "&redirect_uri="
		+ CALLBACK_URL;
	...
	new Thread() {
		...
		try {
		    ...
		    JSONObject jsonObj = (JSONObject) new JSONTokener(streamToString(urlConnection.getInputStream()))
			    .nextValue();
	...
	try {
	    ...
	    JSONArray types = (JSONArray) jsonObj.getJSONArray("notifications");
	    int ilength = venues.length();
	    int is = types.length();
		...
		for (int i = 0; i < is; i++) {
		    ...
		    venue.type = type.getString("type");
		}
